...
//import static boss.Boss.scanLogin;
...
public class Job51 {
    ...
    static String cookiePath = "./src/main/job51/cookie.json";
    ...
    public static void main(String[] args) {
        ...
        if (EnableNotifications) {
            String message = "共投递" + returnList.size() + "个简历，" +
                    "用时" + ((edate.getTime() - sdate.getTime()) / 1000);
...
//            new TelegramNotificationBot().sendMessageWithList(message, returnList, "前程无忧投递");
    ...
    private static Boolean page() {
        ...
        for (int i = 0; i < checkboxes.size(); i++) {
            ...
            log.info("选中：{} | {} 职位", company, titles);
        }
        Thread.sleep(3000);
        ACTIONS.keyDown(Keys.CONTROL).sendKeys(HOME).keyUp(Keys.CONTROL).perform();
    ...
    private static void findAnomaly() {
        try {
            ...
            if (verify.contains("访问验证")){
                //关闭弹窗
                log.error("出现访问验证了！");
                CHROME_DRIVER.quit();//关闭之前的ChroemeCHROME_DRIVER
                System.exit(0);
            }
        } catch (Exception e) {
